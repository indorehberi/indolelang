...
async function main() {
  ...
  // We need an admin user or just any user to act as author for blog
  // and some users for testimonials.
  // First, check if there are users, if not, create dummy users
  ...
  // --- Seed Testimonials ---
  // Delete existing dummy seeds if we want clean state, but just check if count is 0
