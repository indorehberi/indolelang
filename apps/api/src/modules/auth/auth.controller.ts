...
export class AuthController {
	...
	async forgotPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
		try {
			const { email, phone, send_to } = req.body;
			await authService.forgotPassword(email, phone, send_to);
			sendSuccess(res, null, 'Link reset password telah dikirim ke kontak yang Anda pilih jika terdaftar');
		} catch (error) {
			next(error);
		}
	}
