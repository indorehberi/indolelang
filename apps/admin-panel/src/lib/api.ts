 ...
 * On a 401 (expired access token) it silently exchanges the refresh-token
 * cookie for a new access token and retries the request once. If the
 * refresh also fails, the session is cleared and the user is redirected
 * to login instead of the app failing silently.
 */
export async function apiFetch(path: string, options: RequestInit = {}): Promise<Response> {
  ...
  if (response.status === 401 && !isAuthExempt(path)) {
    ...
    // (e.g. an admin/operator role changed elsewhere). Try one silent
    // refresh + retry — refreshAccessToken() re-reads the current role
    // from the DB. If still forbidden afterward, it's a genuine
    // permission error, so don't clear the session — just return as-is.
    const newToken = await refreshAccessToken();
    if (newToken) {
      response = await doFetch(newToken);
    }
